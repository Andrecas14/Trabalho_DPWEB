...
function gerarCalendario($mes, $ano, $eventos) {
    ...
    echo "<table class='table table-bordered text-center' style='table-layout: fixed;'>"; 
    ...
    for ($dia = 1; $dia <= $diasNoMes; $dia++) {
        ...
        echo "<td class='align-middle' style='width: 50px; height: 50px;'>";
        echo "<strong>$dia</strong>";

        if (isset($eventos[$data])) {
            foreach ($eventos[$data] as $evento) {
                echo "<span class='d-inline-block bg-success rounded-circle ms-2' style='width: 14px; height: 14px;' data-bs-toggle='modal' data-bs-target='#eventModal' data-titulo='{$evento['titulo']}' data-descricao='{$evento['descricao']}'></span>";
            }
        }
    ...
    while (($primeiroDia + $diasNoMes) % 7 != 0) {
        echo "<td style='width: 50px; height: 70px;'></td>";
        $diasNoMes++;
    }
